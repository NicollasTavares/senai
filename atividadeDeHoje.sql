...
update cliente set numero = '985' where id=2; 
update cliente set numero = '1950' where id=3; 
update cliente set complemento = 'Bloco H-Ap. 4' where id=3; 
update cliente set numero = '23' where id=4;
update cliente set complemento = 'Casa Fundos' where id=4;
update cliente set numero = '950' where id=5;
update cliente set numero ='26' where id=6;
...
DELIMITER $$
CREATE PROCEDURE PRC_ATUALIZA_VALOR_PEDIDOii (
    PedidoID INT,
    NovoValor DECIMAL(10, 2)
)
BEGIN
UPDATE PEDIDO
    SET valor_total = valor_total + NovoValor
    WHERE ID = PedidoID;
END $$
DELIMITER ; 
...
DROP TRIGGER TRG_ACIONA_PRC_ATUALIZA_VALOR_PEDIDO$$

CREATE TRIGGER TRG_ACIONA_PRC_ATUALIZA_VALOR_PEDIDO
AFTER INSERT ON item_pedido
FOR EACH ROW
BEGIN
    CALL PRC_ATUALIZA_VALOR_PEDIDOii(NEW.pedido_id, NEW.valor_item);
END $$
DELIMITER ;
