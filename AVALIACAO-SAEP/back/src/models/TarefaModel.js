...
class Tarefa {
    ...
    static async listarTarefas() {
        try {
            ...
            const [rows] = await conection.query(`SELECT T.id_tarefa, T.id_usuario, T.descricao,
    ...
    static async atualizarStatus(id, status) {
        try {
            const conn = await connection();
            const Sql = `UPDATE TAREFA SET status=? WHERE id_tarefa=?`;
            const pValues = [status, id];
            const [result] = await conn.query(Sql, pValues)
